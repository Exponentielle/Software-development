...
def negative():
    result = (float(str(eval(ecran_1.get(1.0, END))))*-1)
    ecran_1.delete(1.0, END)
    ecran_1.insert(1.0, result)

def quitter():
    rep = askyesno("Voulez-vous vraiment quitter ?", "Cliquez Oui pour continuer.")
    if rep:
        fen.quit()
...
#ecrans
ecran = Frame(fen, borderwidth=2, width=47, height=4, background=couleurs[0])
ecran.grid(columnspan=4)
ecran_1 = Text(ecran, fg=couleurs[1], background=couleurs[0], width=32, height=2)
ecran_1.grid(row=1, column=1, columnspan=4)
ecran_2 = Label(ecran, background="orange",text=result,font="Bahnschrift 10", fg="green", width=37, height=2)
...
for i in touches:
    if i == touches[0]:
        boutton = Button(fen, bg=couleurs[2], text=i, borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", bd=2, width=17, height=2, command= lambda :efface())
    ...
    if i in touches[6:-4]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda e=i:affiche(e))
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[1]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_pourcent())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i in touches[-3:-1]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
    ...
    if i == touches[-4]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :negative())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[2]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_retirer())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[3]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_inverse())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[4]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_carré())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[5]:
        boutton = Button(fen, bg=couleurs[0], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_racine_carré())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
        colonne += 1
    if i == touches[-1]:
        boutton = Button(fen, bg=couleurs[3], text=i,borderwidth=2, fg=couleurs[1], \
                    activebackground="gray", width=8, height=2, bd=2, command=lambda :calcule_simple())
        boutton.grid(column=colonne, row=ligne, padx=0, pady=1)
    if colonne == 4:
        colonne = 0
        ligne +=1
